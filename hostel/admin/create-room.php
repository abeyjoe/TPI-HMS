...
{
$seater=$_POST['seater'];
$hall=$_POST['hall']; //hall
$roomno=$_POST['rmno'];
$fees=$_POST['fee'];
$sql="SELECT room_no FROM rooms where room_no=?";// added =
$stmt1 = $mysqli->prepare($sql);
$stmt1->bind_param('s',$roomno); // i = s
$stmt1->execute();
$stmt1->store_result(); 
...
{
$query="insert into  rooms (seater,hall,room_no,fees) values(?,?,?,?)"; //hall
$stmt = $mysqli->prepare($query);
$rc=$stmt->bind_param('sssi',$seater,$hall,$roomno,$fees); // iii = sssi $hall
$stmt->execute();
...
<option value="1">Single Seater Bed Space</option>
<option value="2">Two Seater Bed Space</option>
<option value="3">Three Seater Bed Space</option>
<option value="4">Four Seater Bed Space</option>
<option value="5">Five Seater Bed Space</option>
</Select>
</div>
</div>
...
<div class="form-group">
	...
	<Select name="hall" class="form-control" required>
		...
		<option value="OLORI">OLORI HALL</option>
		<option value="ORISUN">ORISUN HALL</option>
		<option value="RAMAT">RAMAT HALL</option>
		<option value="UNITY">UNITY HALL</option>
	</Select>
</div>
</div>
